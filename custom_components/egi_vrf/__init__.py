...
PLATFORMS = ["climate", "button", "sensor"]
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    # Explicitly restore this line (this line is critical and was missing!)
    ...
    poll_interval_seconds = entry.options.get("poll_interval", 2)

    vrf_coordinator = coordinator.EgiVrfCoordinator(
        hass, client, indoor_units, timedelta(seconds=poll_interval_seconds)
    )
    ...
    device = device_registry.async_get_or_create(
        ...
        identifiers={(const.DOMAIN, f"gateway_{entry.entry_id}")},
        name="VRF Gateway",
        manufacturer="EGI",
    ...
    await hass.config_entries.async_forward_entry_setups(entry, ["climate", "button", "sensor"])
