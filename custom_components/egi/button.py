...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    buttons = [
        EgiVrfRescanButton(coord, config_entry, adapter),
    ]

    # Add optional buttons based on adapter capabilities
    if getattr(adapter, "supports_brand_write", False):
        buttons.append(AdapterRestartButton(coord, config_entry, adapter))
        buttons.append(AdapterFactoryResetButton(coord, config_entry, adapter))
